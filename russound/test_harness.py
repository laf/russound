...
def test1():
    x = russound.Russound('192.168.1.72', 9001)
    x.connect()
    zone = '1'
...
def test2():
    ...
    x.connect()
    zone = '1'
    send_msg = x.create_send_message("F0 @cc 00 7F 00 00 @kk 01 04 02 00 @zz 06 00 00", '01', zone)
    x.send_data(send_msg)
    t = 0

    for i in range(1,20):
        print(round(t,1), x.receive_data0())
        time.sleep(0.1)
        t += 0.1
...
def test5():
    """ All on change sound and then all off """
